...
KNOBS=""; for k in MEM_TRACE ALLOC_CONF EMPTY_CACHE LOG_PARAM_BYTES GMU KVMEM MAXLEN SEQS MNBT EAGER CAPTURE_SIZES CAPTURE_SIZES_EXPLICIT CUDAGRAPH_MODE SPEC SPEC_K SPEC_DRAFT SPEC_REJECT SPEC_EXTRA KVOFF_GIB KVFS_DIR KV_RETENTION KV_RELAY KV_RELAY_WINDOW_MIB FI_WORKSPACE_MIB NCCL_LEAN KV_TRACE TEXT_ONLY THINKING ENGRAM_MMAP ENGRAM_THREADS ENGRAM_RELEASE ENGRAM_STATS LOAD_FORMAT INSTANTTENSOR_DRAFT_LOADER DSPARK_DRAFT_PRUNE DSPARK_STATE_DIGEST VLLM_INSTANTTENSOR_MEMAVAIL_MIN_GIB VLLM_INSTANTTENSOR_REF_TRACE EXTRA_ARGS PORT; do
