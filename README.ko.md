...
- **Engram 테이블.** V4.1의 Engram 테이블 203 GB는 체크포인트 파일에 둔 채 읽기 전용 mmap으로 씁니다. 스텝마다 필요한 행을 미리 올리고, 다음 프리필 청크는 백그라운드에서 미리 읽고, 페이지 회수는 커널에 맡깁니다.
...
| Engram | 읽기 전용 mmap 테이블, 스텝별 프리폴트, 백그라운드 프리페치, 디코드 스텝의 프리폴트를 비동기로 걸어 차가운 페이지를 기다리지 않음 |
| GB10 (SM121) | 어텐션, sparse FlashInfer, 인덱서 경로의 페이지 크기 수정, 인덱서 프리필 행을 TP 랭크로 나눠 계산 |
| 스케줄러 | 환경변수로 켜는 프리필 상한(기본은 꺼짐), 긴 프리필 뒤에 밀린 짧은 요청 몫을 프리필 배치에서 예약 |
...
| 프리필, 128K 프롬프트 | 회차에 따라 1,599~1,894 tok/s | 2026-09-16 |
| 디코드, 1스트림, 산문과 코드 | 45.2 tok/s, 86.0 tok/s | 2026-09-16 |
| 128K 프리필 뒤에 밀린 짧은 요청 | 답까지 7~15초 | 2026-09-16 |
