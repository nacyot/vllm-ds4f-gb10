...
**Status (2026-09-07).** Nine commits from a review of MiaAI-Lab's DSpark recipe and upstream main, validated on the 4-node cluster split into two TP=2 pairs (baseline vs candidate) with a 10-check regression suite. Three fork bugs found by that suite are fixed: cold restores of sessions whose length mod 4096 was below 1024 collapsed to a full re-prefill (0 external hits, 70 s; now 118,784 of 119,549 tokens from disk in about 3 s), an exact re-send whose tail past its last 256-token block was 64 tokens or shorter lost the whole prefix cache (now hits the last block), and DSpark acceptance halved for the first steps after such a short-tail hit (0.856 to 0.462; now unchanged). Also in: plain RoPE on the target's two sliding-window layers (vLLM #54815; they were YaRN-scaled), XGrammar #52805/#53046 and KV-offload #54288. Details in the changelog.

**Status (2026-09-03).** Cold start on the 4-node TP=4 production set dropped from **302 s to 137 s** (2-node TP=2: 378 s to 126 s) with the weights proven bit-identical (per-rank SHA-256 of every `state_dict` tensor, both ranks, before and after). Three things were slow: every rank read the whole 156 GiB checkpoint through the mmap loader (85-165 s), the B12X MXFP4 expert repack was memory-bandwidth bound at ~1 s per MoE layer (42-48 s), and the folded DSpark draft re-scanned all 48 shards (23-35 s). They are replaced by InstantTensor (ranks split the read, direct I/O), a fused Triton repack kernel, and index-based shard pruning; the 21-image vision profiling pass is skipped on the production units after measuring what it buys (0.24 s on one first-image request per boot). Warmups and CUDA-graph capture are untouched. Details under the 2026-09-03 changelog entry.

**Status (2026-09-01).** The fork now also serves **DeepSeek-V4-Flash-Vision-Exp**, DeepSeek's first multimodal V4 checkpoint (weights published 2026-08-31), with the same knobs: CPU/disk KV offloading, cold restore, DSpark speculative decoding, tool calling and the reasoning parser all verified with images in the prompt on the GB10 pair. The checkpoint declares the text architecture, so the registry upgrades it to the multimodal wrapper when `vision_n_layers > 0`; text checkpoints are untouched (DeepSeek-V4-Flash-0731 re-verified through the production units on the patched venv). Beyond the community branch (vLLM issue #54561) this implementation applies `bias_vl` in expert selection on every layer including the hash-routed ones, keeps the DSpark/MTP drafters from embedding the out-of-vocabulary image sentinels, and, as a second step, reproduces the reference's **bidirectional attention inside image spans** on the SM120 packed prefill path (`DSPARK_VISION_BIDI`, default on for vision checkpoints). Details in the changelog.
...
- **Reasoning-effort table and tool-turn cap (2026-09-02)**: the DeepSeek V4 encoder keeps DeepSeek's official three-level table (`low` renders nothing, `high` prepends "Reasoning Effort: Absolute maximum with no shortcuts permitted...", `max` prepends "Beyond maximum ... Do not stop reasoning until ..."; identical to the `encoding/encoding_dsv4.py` shipped with the checkpoints and to upstream vLLM). `DSPARK_EFFORT_TIERS=preview` selects the June-preview lineage table (a preamble only for `max`) for A/B runs; a golden test pins the official table to the checkpoint's own `encoding/encoding_dsv4.py`. New `DSPARK_TOOL_EFFORT_CAP=low|high` caps the rendered effort on requests that carry tools (thinking mode only, logged once): the `high` text asks for the entire deliberation to be written out, which on long agent contexts meant 20k-33k reasoning tokens per turn before any tool call (MiaAI-Lab issue #37 saw high+tools fail 3/3 until the preamble was dropped on tool turns). Default unset = official behaviour.
- **Tool-turn sampling (2026-09-02)**: DeepSeek's published agentic operating point is temperature 1.0 with top_p 0.95 (V4-Flash model cards; the reference decoder and DeepSeek Harness have no tool-aware temperature), so `DSPARK_TOOL_TEMP0` is no longer set by default in our units and `DSPARK_TOOL_TOP_P=0.95` applies that top_p to tool-carrying requests that did not send one. The scoped clamp below stays available as a rollback.
- **Tool-turn temperature scope (2026-09-02)**: `DSPARK_TOOL_TEMP0_SCOPE=dsml` (default) keeps the request's own temperature (client value, else the model default) through the `<think>` section and the answer prose, and switches the request to temperature 0 only once the `<｜DSML｜tool_calls>` marker appears (latched for the rest of the request; the first `1 + num_speculative_tokens` structure tokens after the marker are still drawn at the request temperature because the scheduler-side channel is one step behind). `answer` switches at the end of reasoning instead (natural `</think>`, forced end, or the DSML marker), so answer prose is greedy too. The scheduler-side section tracker (`dspark_loop_break.py`) emits the switch on the natural `</think>`, the forced end sequence, or a `<｜DSML｜tool_calls>` marker opened inside the think section, re-sends it every step (self-healing after preemption), and the worker writes it into the sampler's per-request temperature before the forward, so verifier and DSpark drafter agree from the next step. Whole-request greedy decoding of long agent contexts was collapsing into multi-minute semantic reasoning loops. `request` restores the old whole-request zero.
- **Near-repetition detectors (2026-09-02)**: `--reasoning-config` fields `loop_break_novelty_window` (0 = off; e.g. 1024), `loop_break_novelty_min` (0.4), `loop_break_novelty_ngram` (8), `loop_break_novelty_consecutive` (2) add a rolling distinct-n-gram check to the reasoning loop breaker: paraphrase loops and cycles longer than `loop_break_max_pattern_size` never trip the exact-cycle test, but their novelty (share of never-seen 8-grams in the trailing window) collapses from the healthy 0.65-1.0 to below 0.4, and the same forced end sequence follows. `answer_repetition_novelty_window` / `answer_repetition_novelty_min` (0.3) run the statistic over the answer section and finish the request with `finish_reason=repetition` (stop_reason `dspark_answer_repetition(...)`), which caps the greedy answer runaways that otherwise run to `max_tokens`. Thinking-budget hits that land in the same multi-token step as the forced `</think>` are now logged too. Our units set `reasoning_end_str` to a transition phrase ("I have to give the solution based on the reasoning directly now.</think>", the upstream-documented example): vLLM PR #52677 measured that forcing a bare `</think>` lets the loop resume or yields an empty or fabricated answer.
- **Detector metrics (2026-09-02)**: the scheduler-side detectors now export Prometheus counters through `SchedulerStats.dspark_events` → `vllm:dspark_loop_break_fired_total{kind=exact|novelty}`, `vllm:dspark_answer_repetition_total`, `vllm:dspark_thinking_budget_hits_total`, `vllm:dspark_section_temperature_switches_total`; the API-server side adds `vllm:dspark_dsml_leak_guard_stops_total{trigger}` and `vllm:dspark_tool_effort_cap_total{from_effort,to_effort}` (`vllm/v1/metrics/dspark.py`). Together with the per-position DSpark acceptance counters they make the stall fingerprint (pos-0 acceptance flat at 1.0 with a running request) and every forced end visible on a dashboard instead of only in the head log.
- **Answer detector vs. tool-call blocks (2026-09-02, evening)**: the answer-repetition detector no longer finishes a request while a DSML tool-call block is being written, and at the default it does not even feed the block's tokens to its window. Tool-call payloads are legitimately repetitive: a real 2k-token code patch crossed the 0.3 prose threshold at window 1024 and five consecutive agent turns were cut mid-argument (`finish_reason=repetition`; the client saw a truncated tool call and retried). Details: (1) `answer_repetition_dsml_min` (default 0 = off; exact cycles measure ~0.0) is the opt-in threshold from the opener to the end of the section — the closer is not tracked, DeepSeek-V4 ends the turn after it — and the stop_reason carries `section=dsml` when it fires there; (2) the answer window is reset at every section boundary and at the opener, so an in-think tool call (or a block followed by interleaved thinking) cannot pre-load the next answer section with repetitive tokens; (3) the markers are keyed on the `｜DSML｜` special token: the opener's leading `<` merges with a preceding space/period/paren (`" <"`→818, `".<"`→32334), which made an opener that does not start a line invisible; (4) the DSML marker is now tokenized whenever any scheduler-side detector is on — before, only `DSPARK_TOOL_TEMP0=1` did, so at the vendor operating point (TEMP0 unset) the loop breaker's "never force inside a tool-call block" rule was inert too; (5) `vllm:dspark_dsml_blocks_total` counts recognised openers, and the startup line reports `tool-call blocks skipped|min=…|not recognised`. Tests: `tests/v1/core/test_dspark_dsml_block_prod_shape.py` (real ids, every step alignment, merged openers, controls).
- **DSML leak guard scope (2026-09-02)**: `DSPARK_DSML_LEAK_GUARD_SCOPE=answer` (default) leaves the `<think>` section alone, so a model that quotes `</invoke>` or `<dsml:` while reasoning (an agent reading this code, for example) is no longer stopped mid-thought with an empty answer; the guard arms itself when `</think>` appears, or when a `<｜DSML｜tool_calls>` block opens without `</think>` (implicit end, vLLM #48931; the block itself is inspected). Dialect token ids are resolved with the live tokenizer. `request` restores the whole-request check. Every stop now logs `DSPARK DSML leak guard stopped request ...` with the trigger.
- **Boot time (2026-09-03)**: `--load-format instanttensor` with `INSTANTTENSOR_DRAFT_LOADER=auto|safetensors|instanttensor` (port of eugr/spark-vllm-docker's hybrid draft loader: a draft folded into the target checkpoint is not streamed through InstantTensor a second time; `instanttensor` is right once the draft only opens its own shards), `DSPARK_DRAFT_PRUNE=1` (default; the DSpark draft reads `model.safetensors.index.json` and opens only the shards holding `mtp.*`, `embed.weight`, `head.weight`, 5 of 48 for V4-Flash), `DSPARK_B12X_FAST_REPACK=1` (default) with `DSPARK_B12X_REPACK_MODE=fused|batched` (a fused Triton kernel for the W4A16 expert repack, `torch.equal` to b12x's per-expert loop; `DSPARK_B12X_REPACK_VERIFY=1` re-runs the original in-boot and fails on any difference; `DSPARK_B12X_REPACK_CHUNK` experts per launch), `DSPARK_BOOT_TRACE=1` (stage and per-layer post-load timings with `/proc/vmstat` and PSI deltas), `DSPARK_STATE_DIGEST=<prefix>` (write per-tensor SHA-256 of the target and draft `state_dict` at the end of warmup, for validating loader changes). Exact-filename `allow_patterns_overrides` on a model class are now unioned and skip the index completeness check.
...
### 2026-09-07 — cold-restore fix, short-tail prefix hits, draft ring masking, upstream picks (9 commits)

- **Experiment layout.** The four nodes ran as two TP=2 pairs: a = 6040+f323 on the previous HEAD (`c2318ca987`), b = 37cc+27c4 on the candidate, each with the production env scaled to TP=2 (18 GiB KV pool, 524k max length) and its own disk tier. A regression suite (`regress.py` in the notes: sanity, DSpark acceptance, c1/c3/c6 decode, 28k prefill, tool call + guided JSON + named `tool_choice` with thinking, image, ~120k-token needle with an embedded image, warm re-ask, cold restore right after a restart, 16-way soak) ran on both pairs before any change (acceptance 0.34 on both, prefill 1820/1856 tok/s, needle 3/3) and after every step. Restart-to-restart noise on the untouched pair: acceptance 0.34 to 0.32, c1 30 to 29 tok/s.
- **Cold restore of short-tailed sessions was broken** (`262e5fd696`). The cold-restore check (re-ask a 119,549-token needle session right after a restart) got 0 external hits and a 70 s re-prefill on both pairs. The lookup log showed the full-attention and both 64-token window groups hitting (29/29, 116/116, 116/116) and the compressor-state group 0 of 1840, so the reconciliation returned 0 after 522 MB had already been promoted from disk. In the connector lookup the DSpark draft group pops its "volatile trailing chunk" even when the request ends before the extra chunk exists (the query is clamped to the key range), which moves the boundary 1024 tokens below the prompt anchor, where anchor-now (`DSPARK_TAIL_ONLY=2`) keeps no snapshot for the other window groups. Every session whose length mod 4096 is below 1024 was affected; the production TP=4 log holds two such failures. The pop now fires only when the extra chunk was queried and hit (full-attention EAGLE groups keep the unconditional pop); the two unit tests that asserted the old pop were updated (`e92c2e2b17`; 128/128). The same restore now hits 118,784 of 119,549 tokens, loads 0.95 GB and answers in 2.7-3.1 s.
- **An exact re-send with a short tail lost the whole prefix cache** (`2fe8dec3f4`). On the production pair a repeated prompt whose tail past its last 256-token block was 10, 13 or 64 tokens hit 0 of about 1,550 tokens (full re-prefill) while tails of 65 or more hit the last full block. The sliding-window finder needs the EAGLE peek block past the boundary, which lies beyond `num_tokens - 1` for such tails, and under sparse retention the earlier boundaries hold no window run. When the peek block cannot be scanned the finder now takes the plain window run that ends at the boundary (hybrid page sizes only; a single sliding-window group keeps upstream's one-block drop). Test added (90/90). Live: tails 10 and 64 now hit 1,536 tokens.
- **Draft acceptance collapsed after a short-tail hit** (`931b27fd73`). With a 66-token tail the repeated request's DSpark acceptance fell from 0.856 to 0.462 (0.80 at 120 tokens, measured on an idle pair). The draft's 128-position ring of target-hidden-state KV is filled only for positions this request computed, and the mask treated the rest (a previous occupant's KV) as valid context; rejection sampling kept outputs correct, so this cost acceptance only. Each ring row now records the lowest position its request stored (atomic min in the Triton store kernel; the V2 runner marks rows empty when a request is admitted) and the draft attention accepts a slot only if the position it holds exists and is at or above that minimum. Candidate pair: 0.929 to 0.856 at tail 66, 0.856 to 0.856 at 120, unchanged elsewhere.
- **Plain RoPE on the target's sliding-window layers** (`6894aeae32`, vLLM #54815). `resolve_layer_compress_ratio` returned `use_unscaled_rope=False` for every target layer, so layers 0 and 1 (compress-ratio entry 0, pure sliding window) were built with the checkpoint's YaRN (factor 16, original 65,536) while the DSpark draft blocks with the same entry were not; the reference `inference/model.py` disables YaRN for pure sliding-window attention. 11 of the 32 rope frequency pairs change (wavelengths 2.6k-47k tokens, at most 0.037 rad across a 128-token window). The suite is unchanged within noise (needle 3/3, acceptance 0.32-0.34, prefill 1847-1860 tok/s). Existing sliding-window snapshots in the disk tier carry the old K-rope for those two layers; a restore is inconsistent only inside its first 128-token window.
- **Upstream picks.** #52805 and #53046 (XGrammar: ignore token batches after termination, validate post-reasoning-end drafts before advancing the FSM; not in v0.28.1) and #54288 (KV offload: never store the final sampled token's slot, which under speculative decoding holds a rejected draft). All apply cleanly on v0.27.1; their tests pass here. Source: the 2026-09-06/07 review of `MiaAI-Lab/DeepSeek-v4-Flash-DSpark-2x-DGX-Spark` (mialab #222/#213/#223) and upstream main through 2026-09-06.
- **Not yet measured on this code.** Restores of a prompt shorter than the stored session (going back to an earlier turn; the eagle group's pop when its extra chunk is queried and hits is unchanged and may land one boundary lower), several sessions restoring at once, sessions larger than the CPU tier (`DSPARK_SWA_RESERVE` partial restore), and restores while the TTL pruner runs. The August validation of those cases predates today's changes. By design a restore still recomputes from the edit point of an edited conversation and from the nearest surviving anchor when older window snapshots were pruned (`DSPARK_TAIL_KEEP`, TTL).

### 2026-09-03 — cold start 302 s to 137 s: InstantTensor, pruned draft shards, fused B12X repack (1 commit)

- **Where the time went** (`f30471cd42`). Measured on the 4-node TP=4 production boot of 2026-09-02 (302 s from unit start to HTTP): process start and 4-rank NCCL 29 s, target weights 85 s, post-load processing 47 s (8 s on two nodes, 45 s on the other two), DSpark draft load 23 s, `profile_run` 37 s (21 max-size dummy images through the vision encoder plus the 8192-token LM dummy run), kernel warmups 36 s, CUDA-graph capture 23 s. On the 2-node TP=2 pair the same boot was 378 s (weights 164 s, draft 35 s). NVMe was not the bottleneck (`dd iflag=direct` 4.9 GB/s); the stock safetensors path is a single-threaded mmap page-fault walk at 1.8 GiB/s in which every rank materializes every tensor before the weight loader narrows it to its shard, and with 33-70 GiB of RAM available the 156 GiB walk fights page-cache reclaim the whole way.
- **The post-load gap was the B12X W4A16 expert repack, not memory reclaim.** A 1 s sampler of `/proc/vmstat` (direct reclaim, compaction stalls) and PSI stayed flat through the gap; every py-spy sample sat in `b12x/moe/fused/w4a16/prepare.py:_repack_weight`; per-layer timing showed 0.96 s on each of the 42 MoE layers. The loop repacks one expert at a time with 8 gather passes over a 4 MB tensor, so it is memory-bandwidth bound: batching experts with the same torch ops was bit-identical and no faster (w13 0.61 s vs 0.64 s). `b12x_fast_repack.py` fuses the 8 slot passes into one Triton kernel that reads each source int32 once (w13 0.69 s to 0.04 s, w2 0.44 s to 0.02 s, `torch.equal` on real shapes with row rotation; 86/86 in-boot verifications against the original). The b12x package is not modified; the fork installs the batched helpers into it at first use.
- **Loader.** `--load-format instanttensor` (scitix/InstantTensor 0.1.9; no aarch64 wheel on PyPI, builds from source on GB10 in about a minute with the bundled libaio/liburing) reads the checkpoint once per TP group with direct I/O and broadcasts over NCCL: target weights 164 s to 26 s at TP=2 (6.8 GB/s in the progress bar) and 85 s to 26 s at TP=4, uniform across nodes, no page-cache churn, and the drop_caches/compact_memory step before a restart went from about 60 s to about 1 s per node. The DSpark draft then only opens the 5 shards named by the checkpoint index (2.6 s instead of 35 s); with InstantTensor still driving that read, the community's "InstantTensor + dspark reads the model twice" OOM cannot occur.
- **Vision profiling, measured then dropped.** `profile_encoder_cache` runs the vision encoder on 21 max-size dummy images; with `--kv-cache-memory` pinned its memory estimate is unused, so what remains is a warmup. A fresh-boot probe (two text requests first to absorb the LM-side JITs, then the first image request) with and without `--skip-mm-profiling`, twice each, within 0.005 s: first image 1.24 s vs 1.47 s, every later image request identical (0.43-0.65 s), boot 151 s vs 126 s. The production units now skip it (25 s per boot against 0.24 s on one request per boot). The remaining 0.8 s first-image cost exists in both conditions and is not a JIT (no monitor warnings on image requests); not investigated.
- **Validation.** Per-rank SHA-256 of all 2,095 `state_dict` tensors (target and draft) unchanged between the stock loader and every variant (pruned draft, fused repack, InstantTensor target, InstantTensor draft) on both ranks; same greedy answers on the text and image smoke set; the first-request JIT warning set is unchanged; 5-minute idle soak after each boot. Final TP=4 boot on the production set: 137 s (start and NCCL 33, weights 26, repack 7, draft 2, LM dummy run 8, warmups about 17, capture 22). What is left is warmups, capture and process start, which this fork keeps as they are.

### 2026-09-01 — DeepSeek-V4-Flash-Vision-Exp: image input, production feature set, bidirectional image spans (3 commits)

- **Multimodal wrapper, ViT/aligner, image processor, `bias_vl` routing** (`02505ae057`). New `vllm/models/deepseek_v4/{image_processing,vision,mm_preprocess,vision_model,vl_routing}.py`. The image processor is a 1:1 port of the checkpoint's `inference/image_processor.py` (resize solver, 3x3 aligner grid, N-layout, position-dependent leading pad); the whole tokenize-and-expand runs on the token-id path because this fork's DeepSeek-V4 renderer tokenizes text before the multimodal processor. The ViT and aligner are plain modules replicated per rank and verified against the reference tower with the real BF16 tensors. Every MoE gate registers `bias_vl` for vision checkpoints and routes image rows by `topk(scores + bias_vl)` through two kernel passes merged per row (hash layers substitute the sentinels before the `tid2eid` gather); text checkpoints keep the old path. Hash-layer `gate.bias` tensors (dead in the reference) are dropped at load; the DSpark and MTP drafters mask sentinel ids before embedding. The registry resolves `DeepseekV4ForCausalLM` + `vision_n_layers > 0` to the wrapper while keeping the text architecture name so the per-arch fp8 config hook and the `deepseek_v4` tokenizer mode still fire. CPU tests: preprocessing equals the reference on the shipped examples and a size sweep; ViT+aligner numerics against the reference; routing merge against the reference `Gate`.
- **DSpark config**: `SpeculativeConfig` no longer applies the MTP divisibility rule to `method=dspark` (`fbe52a309a`); the vision checkpoint sets `num_nextn_predict_layers=3` with a DSpark block of 5.
- **Bidirectional attention inside image spans** (`b0d87a5432`, `DSPARK_VISION_BIDI`). The reference widens every layer's sliding window inside `[IMAGE_START, IMAGE_END]` so image tokens see the rest of their span (window 128 + up to 384). The FlashInfer SM120 sparse-MLA kernels apply no positional mask, so the per-row index lists are widened to 512; because the >64-row prefill orchestrator only accepts width 128 on the dual-cache layers, image-span segments are tiled into <=64-row decode-kernel calls (the same recipe as `DSPARK_SM120_DECODE_TILE`) and text segments keep today's launch byte for byte. Visibility is computed from the raw ids in the model forward (identical on every TP rank) and cleared after the target forward so the DSpark drafter's prefill stays causal. A startup probe warms the 512-wide kernels and disarms the path on failure; `disable_chunked_mm_input` is forced so an image block is never split across chunks. Requests whose prefix-cache hit resumes inside a span degrade to causal per request (logged). CPU tests compare the visibility math and row construction with the reference `get_image_visible` / `get_window_topk_idxs_visible`.
- Verification on GB10 (TP=2 pair, production env): image answers correct on the shipped examples and synthetic counting/OCR/layout tasks; a 92,197-token prompt with an embedded image built in 53.5 s, follow-ups 1.6-1.8 s from the prefix cache, and after a full restart the same prompt cold-restored from the disk tier with 90,112/92,197 external hits in 2.9 s with the needle and the image both recalled; tool calling with an image, guided JSON, thinking and 8 concurrent image requests pass; DSpark acceptance per drafted token 0.33-0.35 on this checkpoint (0.38 for 0731 on the same prompts). A/B of the bidirectional path against causal on 26 image tasks: identical accuracy (25/26) and a mean answer log-prob difference of -0.007. 0731 regression through the production units: text class resolved, tool calling, guided, 16x500 soak, disk-tier restore, acceptance profile identical to the pre-port logs.
