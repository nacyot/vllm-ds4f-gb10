| `cpu_slabs` | no | `true` | both | With a packed block layout and one host copy per block (replicated layout or `relay_from_rank0`), size CPU rows per KV cache group: one slab per distinct group byte size instead of every row at the largest group. `false` keeps one uniform row size. |
| `cpu_slab_shares` | no | — | both | Relative weight of each slab, keyed by its row bytes as a string (e.g. `{"139264": 0.4, "77824": 0.4, "114688": 0.2}`). Defaults to the bytes one `max_model_len` request leaves in each slab (every chunk for full attention, one window per prefix-cache retention interval otherwise). Per-slab fill is exported as `vllm:kv_offload_cpu_slab_usage_perc{slab_bytes=...}`. |
