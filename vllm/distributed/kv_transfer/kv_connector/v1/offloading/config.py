...
def build_offloading_config(
    ...
    if kv_cache_config.num_blocks > 0:
        ...
        if _compact_packed and is_packed:
            ...
            # (e.g. indexer cache dtype resolved at worker init), so derive
            # the row size from data both sides share: the packed block
            # stride (max group total) times the number of groups, as a
            # consistent upper bound. Page-align at the source so creator
            # and opener compute identical mmap sizes.
            _stride = next(
                t.block_stride
                for t in kv_cache_config.kv_cache_tensors
                if is_kv_cache_tensor_packed(t)
            )
            _total = _stride * len(kv_cache_config.kv_cache_groups)
            _total = (_total + 4095) // 4096 * 4096
            worker_kv_bytes_per_block = _total
