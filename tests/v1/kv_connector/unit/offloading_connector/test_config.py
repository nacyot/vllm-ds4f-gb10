...
def test_group_bytes_bounded_by_block_stride():
    """A merged group spec may report a page larger than the layers it stands
    for (DeepSeek V4 MLA + indexer); the persisted bytes are capped by the
    packed block the group lives in."""
    num_blocks = 4
    spec = FullAttentionSpec(
        block_size=16, num_kv_heads=1, head_size=1, dtype=torch.float32
    )
    page = spec.page_size_bytes
    block_stride = page + page // 2
    packed = KVCacheConfig(
        num_blocks=num_blocks,
        kv_cache_tensors=[
            KVCacheTensor(
    ...
    offloading_config = build_offloading_config(_make_vllm_config(), packed)
    assert offloading_config.packed_layout
    assert offloading_config.worker_kv_bytes_per_block == block_stride
    assert offloading_config.groups[0].bytes_per_block == block_stride
